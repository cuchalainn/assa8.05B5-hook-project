...
int ShouldDoCustomJump() {
    ...
    const wchar_t* commandsToMatch[] = {
        ...
        L"/watch", L"/encount on", L"/encount off", L"/pile"
    };
...
void ProcessParameterHook(DWORD eax_value) {
    ...
    if (wcscmp(finalStringAddress, L"#系統時間秒數差") == 0) {
        SYSTEMTIME st_now, st_year_start = { 0 };
        GetSystemTime(&st_now);
        st_year_start.wYear = st_now.wYear;
        st_year_start.wMonth = 1;
        st_year_start.wDay = 1;

        FILETIME ft_now, ft_year_start;
        SystemTimeToFileTime(&st_now, &ft_now);
        SystemTimeToFileTime(&st_year_start, &ft_year_start);

        ULARGE_INTEGER uli_now, uli_year_start;
        uli_now.QuadPart = *(ULONGLONG*)&ft_now;
        uli_year_start.QuadPart = *(ULONGLONG*)&ft_year_start;
    ...
    else if (wcscmp(finalStringAddress, L"#遊戲時間") == 0) {
        if (!Sa_base) return;
        HANDLE hProcess = OpenProcess(PROCESS_VM_READ, FALSE, Sa_PID);
        if (hProcess) {
            DWORD gameTime = 0;
            LPCVOID dynamicAddress = (LPCVOID)((BYTE*)Sa_base + Addr::GameTimeOffset);
            if (ReadProcessMemory(hProcess, dynamicAddress, &gameTime, sizeof(gameTime), NULL)) {
    ...
    else if (wcscmp(finalStringAddress, L"#NPCID") == 0 || wcscmp(finalStringAddress, L"#WINID") == 0) {
        if (!Sa_base) return;
        HANDLE hProcess = OpenProcess(PROCESS_VM_READ, FALSE, Sa_PID);
        if (hProcess) {
            DWORD windowState = 0;
            LPCVOID dynamicStateAddr = (LPCVOID)((BYTE*)Sa_base + Addr::WindowStateOffset);
            ReadProcessMemory(hProcess, dynamicStateAddr, &windowState, sizeof(windowState), NULL);

            if (windowState == 1) {
                DWORD finalId = 0;
                DWORD idOffset = (wcscmp(finalStringAddress, L"#NPCID") == 0) ? Addr::NpcIdOffset : Addr::WinIdOffset;
                LPCVOID dynamicIdAddr = (LPCVOID)((BYTE*)Sa_base + idOffset);

                ReadProcessMemory(hProcess, dynamicIdAddr, &finalId, sizeof(finalId), NULL);
                swprintf(buffer, 64, L"%u", finalId);
                OverwriteStringInMemory(finalStringAddress, buffer);
            }
            else {
                OverwriteStringInMemory(finalStringAddress, L"視窗未開啟");
            }
            CloseHandle(hProcess);
        }
    }
    else if (wcscmp(finalStringAddress, L"#EV1") == 0) {
        swprintf(buffer, 64, L"%d", ev1);
        OverwriteStringInMemory(finalStringAddress, buffer);
    }
    else if (wcscmp(finalStringAddress, L"#EV2") == 0) {
        swprintf(buffer, 64, L"%d", ev2);
        OverwriteStringInMemory(finalStringAddress, buffer);
    }
    else if (wcscmp(finalStringAddress, L"#EV3") == 0) {
        swprintf(buffer, 64, L"%d", ev3);
        OverwriteStringInMemory(finalStringAddress, buffer);
    }
    else if (wcscmp(finalStringAddress, L"#亂數") == 0) {
        static std::random_device rd;
        static std::mt19937 gen(rd());
        static std::uniform_int_distribution<> distrib(0, 10);
        int randomNumber = distrib(gen);
        swprintf(buffer, 64, L"%d", randomNumber);
        OverwriteStringInMemory(finalStringAddress, buffer);
    }
}
...
void ProcessStringCopyHook(LPCWSTR stringAddress) {
    ...
    if (wcscmp(stringAddress, L"決鬥") == 0) {
         OverwriteStringInMemory(stringAddress, L"決斗");
    }
    else if (wcscmp(stringAddress, L"自動電腦對戰") == 0) {
        OverwriteStringInMemory(stringAddress, L"自動KNPC");
    }
    else if (wcscmp(stringAddress, L"自動戰鬥") == 0) {
        OverwriteStringInMemory(stringAddress, L"自動戰斗");
    }
    else if (wcscmp(stringAddress, L"快速戰鬥") == 0) {
        OverwriteStringInMemory(stringAddress, L"快速戰斗");
    }
    else if (wcscmp(stringAddress, L"登入主機") == 0) {
        OverwriteStringInMemory(stringAddress, L"登陸主機");
	}
	else if (wcscmp(stringAddress, L"登入副機") == 0) {
		OverwriteStringInMemory(stringAddress, L"登陸副機");
	}
	else if (wcscmp(stringAddress, L"登入人物") == 0) {
		OverwriteStringInMemory(stringAddress, L"登陸人物");
	}
	else if (wcscmp(stringAddress, L"自動登入") == 0) {
		OverwriteStringInMemory(stringAddress, L"自動登陸");
	}
	else if (wcscmp(stringAddress, L"走動遇敵") == 0) {
		OverwriteStringInMemory(stringAddress, L"功能取消");
	}
    else if (wcscmp(stringAddress, L"走動步數") == 0) {
        OverwriteStringInMemory(stringAddress, L"功能取消");
    }
    else if (wcscmp(stringAddress, L"快速遇敵") == 0) {
        OverwriteStringInMemory(stringAddress, L"功能取消");
    }
    else if (wcscmp(stringAddress, L"快速延遲") == 0) {
        OverwriteStringInMemory(stringAddress, L"功能取消");
    }
    else if (wcscmp(stringAddress, L"自動猜謎") == 0) {
        OverwriteStringInMemory(stringAddress, L"功能取消");
    }
    else if (wcscmp(stringAddress, L"腳本延遲") == 0) {
        OverwriteStringInMemory(stringAddress, L"腳本延時");
    }
    else if (wcscmp(stringAddress, L"戰鬥補氣") == 0) {
        OverwriteStringInMemory(stringAddress, L"戰斗補氣");
    }
